...
SETUP (one time, and it needs a human in a browser):
  ...
  1. Play Console -> Create app -> PoFootball. The application id below has to be
...
def main():
    ...
    parser.add_argument("--aab", default=DEFAULT_AAB,
                        help="path to the .aab (default: Builds/Android/PoFootball.aab)")
    parser.add_argument("--credentials",
                        default=os.environ.get("POFOOTBALL_PLAY_CREDENTIALS",
                                               DEFAULT_CREDENTIALS),
    ...
    if not os.path.exists(args.aab):
        fail("no AAB at " + args.aab + " — run PoFootball → Build Android AAB first")
