# PoFootball — Google Play Console listing
...
| Field | Value |
|---|---|
| App name | PoFootball: AI Football Sim |
...
`com.punkoutersoftware.pofootball` — **CHANGE REQUIRED**, the project currently has `com.punkouter.pofootball (not set for Android)`
...
```
PoFootball: AI Football Sim
```

**Short description** (79/80)

```
American football simulated by neural nets that learned the game by playing it.
```

**Full description** (750/4000)

```
PoFootball is an American football simulation you watch rather than control. Players are flat shapes whose geometry tells you the position — quarterback, receiver, lineman, defender — and their behaviour comes from neural networks trained by self-play rather than from a playbook someone typed in.

• Whole-team behaviour learned by self-play, not hand-authored plays
• Position is readable at a glance from each player's shape
• A simulation to watch — the drama is in what the brains decide to do
• Runs entirely on your device; nothing is uploaded anywhere

PoRacer, PoSumo, PoBox, PoCross, PoDance, PoFlag and PoSoccer are all built the same way: a physics simulation, and behaviour that was learned rather than written.
...
| App category | Game |
| Category | Sports |
| Tags | Sports, Simulation, Artificial Intelligence, Strategy, Single player |
...
|---|---|
| Category | Game |
| Violence | None — abstract shapes, no depicted contact injury |
...
| Release name | PoFootball: AI Football Sim 0.1.0 (1) |
...
| Item | State |
|---|---|
| Application id | `com.punkouter.pofootball (not set for Android)` — needs changing to `com.punkoutersoftware.pofootball` |
| Company name | `punkouter` — should be `Punkouter Software` |
...
Project note: No Android application id set. Portrait 9:16 already. Boots SCN_MENU.
